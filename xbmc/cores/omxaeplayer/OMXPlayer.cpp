...
retry:
  ...
  {
    CLog::Log(LOGINFO, "COMXAEPlayer::OpenInputStream - DVD/BD not supported - Will try...");
    // return false;
  }
...
void COMXAEPlayer::OpenDefaultStreams(bool reset)
{
...
bool COMXAEPlayer::WaitForPausedThumbJobs(int timeout_ms)
{
    ...
    */
    ...
    if (CJobManager::GetInstance().IsProcessing(kJobTypeMediaFlags) > 0)
    {
      if (!WaitForPausedThumbJobs(20000))
      {
        ...
        CLog::Log(LOGINFO, "COMXAEPlayer::Process:thumbgen jobs still running !!!");
      }
    }
    */
...
bool COMXAEPlayer::CheckDelayedChannelEntry(void)
{
  bool bReturn(false);
...
int64_t COMXAEPlayer::GetTotalTime()
{
...
bool COMXAEPlayer::OpenAudioStream(int iStream, int source, bool reset)
{
...
bool COMXAEPlayer::OpenVideoStream(int iStream, int source, bool reset)
{
...
bool COMXAEPlayer::ShowPVRChannelInfo(void)
{
  bool bReturn(false);
...
bool COMXAEPlayer::SwitchChannel(const CPVRChannel &channel)
{
...
bool COMXAEPlayer::CachePVRStream(void) const
{
...
void COMXAEPlayer::GetRenderFeatures(std::vector<int> &renderFeatures)
{
  renderFeatures.push_back(RENDERFEATURE_STRETCH);
  renderFeatures.push_back(RENDERFEATURE_CROP);
}

void COMXAEPlayer::GetDeinterlaceMethods(std::vector<int> &deinterlaceMethods)
{
  deinterlaceMethods.push_back(VS_INTERLACEMETHOD_DEINTERLACE);
}

void COMXAEPlayer::GetDeinterlaceModes(std::vector<int> &deinterlaceModes)
{
...
void COMXAEPlayer::GetScalingMethods(std::vector<int> &scalingMethods)
{
}

void COMXAEPlayer::GetAudioCapabilities(std::vector<int> &audioCaps)
{
...
void COMXAEPlayer::GetSubtitleCapabilities(std::vector<int> &subCaps)
{
